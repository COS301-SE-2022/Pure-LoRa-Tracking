...
export class AiParticleFilterService extends AiProcessingStrategyService {
  ...
  constructor(
    public locationComputations: LocationService,
    protected serviceBus: ProcessingApiProcessingBusService
  ) {
    super(serviceBus);
  ...
  changeGateways(gateways: { latitude: number; longitude: number }[]) {
    if (gateways.length < 2) throw 'Not enough gateways given';
  ...
  generatePolygonSamples(howMany: number): boolean {
    const polygon = {
      ...
      geometry: {
        type: 'Polygon',
        coordinates: [this.reservePolygon],
      },
    };
  ...
  weightsMeasuredRelativeToOriginal?(originalPoint: number[]): number[] {
    ...
    for (let k = 0; k < this.gatewayLocations.length; k++)
      originalPointMeasure.push(
        this.distanceBetweenCoords(originalPoint, this.gatewayLocations[k])
      );

    for (let i = 0; i < n; i++) {
      const randomParticlesToCompare = [];
      for (let j = 0; j < this.gatewayLocations.length; j++) {
        randomParticlesToCompare.push(
          this.distanceBetweenCoords(
            this.particles[i],
            this.gatewayLocations[j]
          )
        );
      }
      this.weights[i] = this.weightDistanceEuclidean(
        originalPointMeasure,
        randomParticlesToCompare
      );
    }
    return this.weights;
  }

  weightDistanceEuclidean?(
    OriginalPoint: number[],
    RandomParticle: number[]
  ): number {
    let sums = 0;
  ...
  cumulativeWeights(): number[] {
    const cumulativeWeightArray = [],
      weightShadowArray = [];

    for (let i = 0; i < this.weights.length; i++) {
      weightShadowArray.push(this.weights[i]);
      cumulativeWeightArray.push(
        weightShadowArray.reduce((cumulative, a) => cumulative + a, 0)
      );
    }
  ...
  /*
    ...
    */
  async generateNewSampleFromWeights(
    points = this.particles,
    weights = this.weights
  ): Promise<number[][]> {
    if (points.length != weights.length)
      throw Error(
        'Point and Weight dimension incorrect: P-' +
  ...
  async particleFilter?(reading: {
    ...
    if (reading.gateways != undefined) this.changeGateways(reading.gateways);

    if (this.reservePolygon.length < 3) throw 'No reserve set';
...
export class particleFilterStratifiedService extends AiParticleFilterService {
  constructor(
    locationComputations: LocationService,
    protected serviceBus: ProcessingApiProcessingBusService
  ) {
  ...
  resampleParticles(): void {
    ...
    while (n < this.numberOfSamples) {
      const u0 =
        Math.random() *
          (1 / this.numberOfSamples + 0.000000000001 - 0.000000000001) +
        0.000000000001;
...
export class particleFilterMultinomialService extends AiParticleFilterService {
  ...
  resampleParticles(howMany?: number): void {
    ...
    while (n < this.numberOfSamples) {
      const u =
        Math.random() * (1 + 0.000000000001 - 0.000000000001) + 0.000000000001;
      let m = 1;
