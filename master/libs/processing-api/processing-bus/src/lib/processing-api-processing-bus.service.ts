...
export class ProcessingApiProcessingBusService {
  ...
  async saveDevicePerimeterToDB(body: {
    perimeter: number[];
    name: string;
    deviceID: string;
  }) {
  ...
  async updateDevicePerimeter(body: {
    deviceID: string;
    perimeter: number[];
    name: string;
  }) {
  ...
  async markProcessed(
    data: { deviceEUI: string; timestamp: string }[]
  ): Promise<void> {
